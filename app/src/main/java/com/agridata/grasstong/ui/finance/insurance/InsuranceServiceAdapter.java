 ...
 * @Description :
 */
public class InsuranceServiceAdapter extends BaseRecyclerViewAdapter<GrassBean, BaseRecyclerViewHolder> {
    final  private List<GrassBean> mDates;
   final private Context mContext;


    public InsuranceServiceAdapter(int layoutResId, List<GrassBean> dataList, Context context) {
    ...
    protected void convert(BaseRecyclerViewHolder viewHolder, GrassBean data, final int position) {
        ...
        switch (position) {
            case 0:
                imageView.setImageDrawable(ContextCompat.getDrawable(mContext,R.drawable.ic_insurance_price));
                break;
            case 1:
                imageView.setImageDrawable(ContextCompat.getDrawable(mContext,R.drawable.ic_insurance_quality));
                break;
